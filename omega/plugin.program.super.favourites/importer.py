...
def extractAll(filename, dp, location):
    ...
    profile = os.path.join(root, 'Super Favoritos')
    ...
    if IMPORT_RESET:
        try:    sfile.rmtree(os.path.join(ROOT, 'Super Favoritos'))
        except: pass
        ...
        for item in zin.infolist():
            ...
            if filename == 'settings.xml':
                ...
                try:
                    ...
                    utils.log(e)
            elif filename.lower().startswith('super favoritos'):
                zin.extract(item, root)
