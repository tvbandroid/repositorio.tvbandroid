...
def do_downloadpage(url, post=None, headers=None):
    ...
    if not data:
        if not '/?s=' in url:
            if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title = 'Episodios', action = 'last_epis', url = host + 'episodio/', group = 'Epis', search_type = 'tvshow' ))
...
def idiomas(item):
    ...
    itemlist.append(item.clone( title = 'En castellano', action = 'list_all', url = host + 'audios/castellano/', text_color = text_color ))

    itemlist.append(item.clone( title = 'En latino', action = 'list_all', url = host + 'audios/latino/', text_color = text_color ))

    itemlist.append(item.clone( title = 'Subtitulado', action = 'list_all', url = host + 'audios/sub-espanol/', text_color = text_color ))
...
def news_epis(item):
    ...
    for match in matches:
        ...
        season = scrapertools.find_single_match(match, '<span class="sideranking-episode-label">T(.*?)x').strip()

        if not season: season = 1

        epis = scrapertools.find_single_match(match, '<span class="sideranking-episode-label">.*xE(.*?)</span>').strip()
...
def episodios(item):
    ...
    if item.page == 0 and item.perpage == 50:
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 100:
                platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando ' + str(sum_parts) + ' elementos[/COLOR]')
        elif tvdb_id:
            if sum_parts > 50:
                platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando Todos los elementos[/COLOR]')
                item.perpage = sum_parts
        else:
            item.perpage = sum_parts
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('DoramaExpress', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
...
def findvideos(item):
    ...
    for match in matches:
        ...
        if url:
            ...
            if '/es.' in match: lang = 'Esp'
            elif '/mx.' in match: lang = 'Lat'
