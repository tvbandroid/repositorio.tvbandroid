...
def idiomas(item):
    ...
    itemlist.append(item.clone( title = 'En Latino', action = 'list_all', url = host + 'animes?genero=anime-latino', text_color='moccasin' ))
...
def generos(item):
    ...
    for value, title in matches:
        if value == 'anime-latino': continue
        elif value == 'castellano': continue
...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<button>Filtrar</button>(.*?)$')
    ...
    for match in matches:
        ...
        tipo = 'movie' if '>Pelicula<' in match or '?tipo=pelicula' in item.url else 'tvshow'
...
def list_last(item):
    ...
    for match in matches:
        ...
        tipo = 'movie' if epis == '0' or '>Película<' in match else 'tvshow'
