...
def _proxies(item):
    ...
    elif item.from_channel == 'mega1link':
        from channels import mega1link
        item.channel = 'mega1link'
        mega1link.configurar_proxies(item)

        if config.get_setting('channel_mega1link_proxies') is None: refrescar = False
