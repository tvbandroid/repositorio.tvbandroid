...
class Navigator:
	...
	def main(self):
		def _process():
			for count, item in enumerate(browse_list):
				try:
					...
					cm_items = [
					('[B]Mover[/B]', self.run_plugin % self.build_url({'mode': 'menu_editor.move', 'active_list': self.list_name, 'position': count})),
		...
		if not results and browse_list:
			k.logger('PlayTVBan', 'Construcción del menú vacía para %s (se esperaban %s elementos)' % (self.list_name, len(browse_list)))
	...
	def my_lists(self):
		...
		if s.trakt_user_active():
			self._safe_add({'mode': 'navigator.trakt_lists_personal'}, 'Listas Trakt', 'trakt')
		self._safe_add({'mode': 'navigator.trakt_lists_public'}, 'Listas Públicas de Trakt', 'trakt')
		if s.tmdblist_user_active():
			self._safe_add({'mode': 'navigator.tmdb_lists_personal'}, 'Listas TMDb', 'tmdb')
	...
	def tmdb_watchlists(self):
		...
		self.add({'mode': 'tmdblist.build_tmdb_list', 'list_id': 'watchlist', 'media_type': 'movie', 'list_name': 'Películas - Ver más tarde'}, 'Películas - Ver más tarde', 'tmdb')
		self.add({'mode': 'tmdblist.build_tmdb_list', 'list_id': 'watchlist', 'media_type': 'tv', 'list_name': 'Series - Ver más tarde'}, 'Series - Ver más tarde', 'tmdb')
	...
	def trakt_lists_personal(self):
		...
		self.add({'mode': 'navigator.trakt_recommendations', 'category_name': 'Recomendados'}, 'Recomendados', 'trakt')
		self.add({'mode': 'build_my_calendar'}, 'Calendario', 'trakt')
		if s.trakt_user_active():
			self.add({'mode': 'navigator.search_history', 'action': 'trakt_my_lists'}, 'Buscar en Mis Listas de Trakt', 'search')
		self._set_exit_params({'mode': 'navigator.my_lists'})
		self.end_directory()
	...
	def random_lists(self):
		...
		self.add({'mode': 'navigator.build_random_lists', 'menu_type': 'because_you_watched'}, 'Aleatorio Basado en lo que Has Visto', 'because_you_watched')
		if s.tmdblist_user_active():
			self.add({'mode': 'navigator.build_random_lists', 'menu_type': 'tmdb_lists'}, 'Listas Aleatorias de TMDb', 'tmdb')
		self.add({'mode': 'navigator.build_random_lists', 'menu_type': 'personal_lists'}, 'Listas Personales Aleatorias', 'lists')
		if s.simkl_user_active():
			self.add({'mode': 'navigator.build_random_lists', 'menu_type': 'simkl_lists'}, 'Listas Aleatorias de Simkl', 'simkl')
		if s.trakt_user_active():
	...
	def simkl_lists(self):
		...
		for url_params, label in (
			(self._simkl_list_link('build_movie_list', 'simkl_plantowatch', 'Películas Pendientes de Ver'), 'Películas Pendientes de Ver'),
			(self._simkl_list_link('build_tvshow_list', 'simkl_plantowatch', 'Series Pendientes de Ver'), 'Series Pendientes de Ver'),
	...
	def simkl_watchlists(self):
		self.category_name = 'Pendiente de Ver'
		self._safe_add(self._simkl_list_link('build_movie_list', 'simkl_plantowatch', 'Películas Pendientes de Ver'), 'Películas', 'simkl')
		self._safe_add(self._simkl_list_link('build_tvshow_list', 'simkl_plantowatch', 'Series Pendientes de Ver'), 'Series', 'simkl')
		self.end_directory()
	...
	def mdblist_lists(self):
		...
		self._safe_add({'mode': 'mdblist.get_mdbl_liked_lists', 'name': 'Listas de Películas Favoritas', 'media_type': 'movie'}, 'Listas de Películas Favoritas', 'mdblist')
		self._safe_add({'mode': 'mdblist.get_mdbl_liked_lists', 'name': 'Listas de Series Favoritas', 'media_type': 'tvshow'}, 'Listas de Series Favoritas', 'mdblist')
		self._safe_add({'mode': 'mdblist.get_mdbl_top_lists', 'name': 'Listas Populares de MDBList'}, 'Listas Populares de MDBList', 'mdblist')
		self._set_exit_params({'mode': 'navigator.my_lists'})
		self.end_directory()
	...
	def tools(self):
		...
		if get_setting('redlight.external_scraper.module') not in ('empty_setting', ''):
			self.add({'mode': 'open_external_scraper_settings', 'isFolder': 'false'}, 'Configuración del Scraper Externo', 'settings')
	...
	def import_export(self):
		self.add({'mode': 'settings_backup.import_settings', 'isFolder': 'false'}, 'Importar Configuración de PlayTVBan', 'settings')
		self.add({'mode': 'settings_backup.export_settings', 'isFolder': 'false'}, 'Exportar Configuración de PlayTVBan', 'settings')
	...
	def maintenance(self):
		...
		self.add({'mode': 'sync_settings', 'silent': 'false', 'isFolder': 'false'}, 'Reconstruir la caché de ajustes', 'settings')
		...
		self.add({'mode': 'search.clear_search', 'isFolder': 'false'}, 'Borrar historial de búsquedas', 'settings')
		...
		self.add({'mode': 'clear_cache', 'cache': 'easynews_scrape', 'isFolder': 'false'}, 'Borrar caché de búsquedas de EasyNews', 'settings')
		...
		self.add({'mode': 'clear_cache', 'cache': 'ad_cloud', 'isFolder': 'false'}, 'Borrar caché de AllDebrid', 'settings')
	...
	def set_view_modes(self):
		...
		self.add({'mode': 'navigator.choose_view', 'view_type': 'view.episodes', 'content': 'episodes'}, 'Configurar episodios', 'next_episodes')
		self.add({'mode': 'navigator.choose_view', 'view_type': 'view.episodes_single', 'content': 'episodes', 'name': 'episode lists'}, 'Configurar listas de episodios', 'calender')
	...
	def changelog_utils(self):
		...
		self.add({'mode': 'show_text', 'heading': 'Registro de cambios', 'file': redlight_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Registro de cambios', 'lists')
	...
	def shortcut_folders(self):
		...
		if folders:
			...
			self.add({'mode': 'menu_editor.shortcut_folder_make', 'isFolder': 'false'}, '[I]Crear nueva carpeta...[/I]', 'new')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				cm_items = [
				('[B]MOVER[/B]', self.run_plugin % self.build_url({'mode': 'menu_editor.shortcut_folder_edit', 'active_list': list_name, 'position': count, 'action': 'move'})),
	...
	def _safe_add(self, url_params, list_name, iconImage='folder', original_image=False, cm_items=[]):
		...
		except Exception as e: k.logger('PlayTVBan', 'my_lists add failed [%s]: %s' % (list_name, e))
