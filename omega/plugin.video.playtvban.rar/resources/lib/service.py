...
class SetAddonConstants:
	def run(self):
		kodi_utils.logger('PlayTVBan', 'SetAddonConstants Service Starting')
		import random
		...
		if prev_version and prev_version != new_version:
			...
			kodi_utils.logger('PlayTVBan', 'SetAddonConstants - version %s -> %s' % (prev_version, new_version))
		...
		return kodi_utils.logger('PlayTVBan', 'SetAddonConstants Service Finished')

class DatabaseMaintenance:
	def run(self):
		kodi_utils.logger('PlayTVBan', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import check_databases_integrity
		check_databases_integrity(silent=True)
		return kodi_utils.logger('PlayTVBan', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		kodi_utils.logger('PlayTVBan', 'SyncSettings Service Starting')
		from caches.settings_cache import sync_settings, settings_sync_needed
		if not settings_sync_needed():
			return kodi_utils.logger('PlayTVBan', 'SyncSettings Service Skipped')
		sync_settings({'load_properties': False})
		return kodi_utils.logger('PlayTVBan', 'SyncSettings Service Finished')
...
class BootstrapSettings:
	def run(self, monitor):
		...
		if not service_bootstrap_needed():
			return
		kodi_utils.logger('PlayTVBan', 'BootstrapSettings Service Starting')
		...
		return kodi_utils.logger('PlayTVBan', 'BootstrapSettings Service Finished')
...
def run_deferred_service_setup():
	global _custom_windows_thread_started
	kodi_utils.logger('PlayTVBan', 'Deferred Service Setup Starting')
	try:
		from windows.base_window import ExtrasUtils
		ExtrasUtils().run()
	except Exception as e: kodi_utils.logger('DeferredServiceSetup', 'ExtrasUtils: %s' % e)
	return kodi_utils.logger('PlayTVBan', 'Deferred Service Setup Finished')

class CustomWindowsPrepare:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'CustomWindowsPrepare Service Starting')
		...
		return kodi_utils.logger('PlayTVBan', 'CustomWindowsPrepare Service Finished')

class TraktMonitor:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('PlayTVBan', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				elif status == 'no_auth': kodi_utils.logger('PlayTVBan', trakt_service_string % ('Not Run. No Current Trakt Account', next_update_string))
				else:
					if status in ('success', 'no account'):
						kodi_utils.logger('PlayTVBan', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else:
						kodi_utils.logger('PlayTVBan', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
			...
			except Exception as e: kodi_utils.logger('PlayTVBan', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del player
		except: pass
		return kodi_utils.logger('PlayTVBan', 'TraktMonitor Service Finished')

class SimklMonitor:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'SimklMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('PlayTVBan', 'Simkl Sync Failed')
				elif status == 'no_auth': kodi_utils.logger('PlayTVBan', 'Simkl Sync Not Run - No Account')
				else: kodi_utils.logger('PlayTVBan', 'Simkl Sync %s - %s' % ('OK' if status == 'success' else 'No Changes', next_update_string))
				if status == 'success' and get_setting('redlight.simkl.refresh_widgets', 'false') == 'true' and not kodi_utils.service_shutting_down(monitor):
					kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('PlayTVBan', 'Simkl Sync Failed: %s' % str(e))
			wait_for_abort(wait_time)
		try: del player
		except: pass
		return kodi_utils.logger('PlayTVBan', 'SimklMonitor Service Finished')

class MdblistMonitor:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'MDBListMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('PlayTVBan', 'MDBList Sync Failed')
				elif status == 'no_auth': kodi_utils.logger('PlayTVBan', 'MDBList Sync Not Run - No Account')
				else: kodi_utils.logger('PlayTVBan', 'MDBList Sync %s - %s' % ('OK' if status == 'success' else 'No Changes', next_update_string))
				if status == 'success' and get_setting('redlight.mdblist.refresh_widgets', 'false') == 'true' and not kodi_utils.service_shutting_down(monitor):
					kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('PlayTVBan', 'MDBList Sync Failed: %s' % str(e))
			wait_for_abort(wait_time)
		try: del player
		except: pass
		return kodi_utils.logger('PlayTVBan', 'MDBListMonitor Service Finished')

class WidgetRefresher:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'WidgetRefresher Service Starting')
		from time import time
		player = kodi_utils.kodi_player()
		...
		while not monitor.abortRequested():
			try:
				...
				if self.next_refresh < time():
					kodi_utils.logger('PlayTVBan', 'WidgetRefresher Service - Widgets Refreshed')
					kodi_utils.refresh_widgets()
					self.set_next_refresh(time())
			except: pass
		try: del player
		except: pass
		return kodi_utils.logger('PlayTVBan', 'WidgetRefresher Service Finished')
...
class AutoStart:
	def run(self, monitor):
		kodi_utils.logger('PlayTVBan', 'AutoStart Service Starting')
		...
		return kodi_utils.logger('PlayTVBan', 'AutoStart Service Finished')

class AddonXMLCheck:
	def run(self):
		kodi_utils.logger('PlayTVBan', 'AddonXMLCheck Service Starting')
		try: kodi_utils.reuse_language_invoker_check()
		except Exception as e: kodi_utils.logger('AddonXMLCheck', str(e))
		return kodi_utils.logger('PlayTVBan', 'AddonXMLCheck Service Finished')
...
class RedLightMonitor(Monitor):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			kodi_utils.logger('OnNotificationActions', 'PAUSANDO los Servicios de PlayTVBan Debido al Modo de Suspensión del Dispositivo')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			kodi_utils.clear_property(pause_services_prop)
			kodi_utils.logger('OnNotificationActions', 'REANUDANDO los Servicios de PlayTVBan Tras Activar el Dispositivo')
			...
			try:
				...
				if info.get('id') == 'plugin.video.playtvban':
					...
					kodi_utils.logger('PlayTVBan', 'Cierre del servicio: el addon se ha deshabilitado para actualizarse o desinstalarse')
			except: pass
...
if __name__ == '__main__':
	...
	kodi_utils.logger('PlayTVBan', 'Main Monitor Service Starting')
	RedLightMonitor().waitForAbort()
	kodi_utils.logger('PlayTVBan', 'Main Monitor Service Finished')