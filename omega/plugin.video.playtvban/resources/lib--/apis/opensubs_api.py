...
def check_account():
	if not st.opensubs_configured():
		return ku.ok_dialog(heading='OpenSubtitles', text='Introduce primero tu nombre de usuario y contraseña de OpenSubtitles.')
	try:
		response = requests.post('%s/login' % BASE_URL, headers=_headers(), json={'username': _username(), 'password': _password()}, timeout=TIMEOUT)
		if response.status_code != 200:
			return ku.ok_dialog(heading='OpenSubtitles', text='Error de inicio de sesión. Comprueba tu nombre de usuario y contraseña de OpenSubtitles.')
		data = response.json()
		...
		return ku.ok_dialog(heading='OpenSubtitles', text='Cuenta: %s[CR][CR]Descargas permitidas (24 h): %s' % (_username(), allowed))
	except:
		return ku.ok_dialog(heading='OpenSubtitles', text='Error al verificar la cuenta de OpenSubtitles. Comprueba tu nombre de usuario y contraseña.')
...
def revoke_access():
	...
	return ku.ok_dialog(heading='OpenSubtitles', text='Se han borrado el nombre de usuario, la contraseña y la información de inicio de sesión guardada de OpenSubtitles.')
