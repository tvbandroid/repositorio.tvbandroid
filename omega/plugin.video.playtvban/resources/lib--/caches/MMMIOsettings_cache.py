...
def set_string(params):
	...
	if setting_id == 'tmdb_api' and new_value and looks_like_tmdb_v4_jwt(new_value):
		kodi_utils.ok_dialog(heading='Tipo de Clave incorrecto', text='Este es un Token de Acceso de Lectura de TMDb v4 (JWT), no una Clave API v3.[CR]Utiliza TMDb Lists → Token de Acceso de Lectura para los Tokens v4.')kodi_utils.ok_dialog(heading='Tipo de Clave incorrecto',text='Este es un Token de Acceso de Lectura de TMDb v4 (JWT), no una Clave API v3.[CR]Utiliza TMDb Lists → Token de Acceso de Lectura para los Tokens v4.')
		return set_string(params)
...
def set_from_list(params):
	...
	if setting_id == 'external_scraper.run_mode' and setting_value != '1':
		...
		warning_text = (
			...
			'[B]%s[/B] puede consultar los mismos Indexadores varias veces. [B]Series (Alternativa por Orden de Prioridad)[/B] es la Opción Predeterminada Recomendada.[CR][CR]'
