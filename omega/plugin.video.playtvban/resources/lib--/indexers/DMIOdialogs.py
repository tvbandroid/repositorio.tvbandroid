...
def language_invoker_choice(params):
	...
	if not kodi_utils.confirm_dialog(text='Activar [B]Reutilizar Invocador de Idioma[/B] %s?' % ('On' if new_value == 'true' else 'Off')): return
	if new_value == 'true' and not kodi_utils.confirm_dialog(text='Habilitar esta configuración puede causar inestabilidad en algunos dispositivos.[CR][CR]Continuar?'): return
...
def _tmdblists_manager_choice(params):
	...
	if action == 'list_add_new':
		...
		if not list_id: return kodi_utils.notification('Error Al Crear La Lista')
...
def favorites_manager_choice(params):
	...
	if success:
		...
		kodi_utils.notification('Completo', 3500)
...
def ai_model_order_choice(params):
	...
	kwargs = {'items': json.dumps(choices), 'multi_line': 'true', 'heading': 'Seleccione El Orden De Clasificación De Los Modelos De IA'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice == None: return
...
def extras_lists_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Habilitar Contenido para Listas Adicionales', 'multi_choice': 'true', 'preselect': preselect}
	selection = kodi_utils.select_dialog(choices, **kwargs)
	if selection  == []:
		kodi_utils.ok_dialog(text='Debes seleccionar al menos 1 elemento')
...
def extras_order_choice(params={}):
	...
	kwargs = {'items': json.dumps(choices), 'heading': 'Elegir Orden de las Listas Extras', 'multi_line': 'true', 'narrow_window': 'true'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice == None:
	...
	display_position = choice['display_position']
	choices = [{'line1': item['name'], 'value': item['value']} for item in active_items if item != current_item]
	kwargs = {'items': json.dumps(choices), 'narrow_window': 'true', 'heading': 'Elegir Elemento para la Posición %02d' % display_position}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice != None:
		value = choice['value']
		current_order.remove(value)
...
def preferred_filters_choice(params):
	...
	def _default_choices():
		return [{'name': '1ª Prioridad', 'value': 'Elegir 1er Parámetro de Ordenación'},
				{'name': '2ª Prioridad', 'value': 'Elegir 2º Parámetro de Ordenación'},
				{'name': '3ª Prioridad', 'value': 'Elegir 3er Parámetro de Ordenación'},
				{'name': '4ª Prioridad', 'value': 'Elegir 4º Parámetro de Ordenación'},
				{'name': '5ª Prioridad', 'value': 'Elegir 5º Parámetro de Ordenación'}]
	...
	def _rechoose_checker(choice):
		...
		clear_choice = kodi_utils.confirm_dialog(
			heading='Parámetro Actual Activo',
			text='Esta posición de ordenación ya está ocupada.[CR]Por favor, elige qué acción realizar.',
			ok_label='Reemplazar',
			cancel_label='Limpiar')
	...
	def _param_choices(choice):
		...
		param_kwargs = {'items': json.dumps(param_list_items), 'multi_line': 'true', 'heading': 'Elegir Parámetros para Priorizar en Reproducción Automática', 'narrow_window': 'true'}
	...
	kwargs = {'items': json.dumps(list_items), 'multi_line': 'true', 'heading': 'Elegir Parámetros para Priorizar en Reproducción Automática', 'narrow_window': 'true'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
...
def tmdb_api_check_choice(params):
	...
	if looks_like_tmdb_v4_jwt(api_key):
		return kodi_utils.ok_dialog(
			heading='Tipo de clave incorrecto',
			text='Esta es una Read Access Token v4 (JWT) de TMDb, no una API Key v3.[CR]Usa TMDb Lists → Read Access Token para los tokens v4.')
	data = movie_details('299534', api_key)
	if not data or not data.get('success', True):
		text = 'La API Key de TMDb ha fallado.[CR]%s' % (data or {}).get('status_message', 'Error desconocido')
		return kodi_utils.ok_dialog(heading='Error', text=text)
	return kodi_utils.ok_dialog(heading='Correcto', text='La API Key de TMDb es válida.')
...
def tmdblist_read_token_check_choice(params):
	...
	try:
		...
		if not data.get('success'):
			text = 'El token de acceso de lectura de listas ha fallado.[CR]%s' % data.get('status_message', 'Error desconocido')
			return kodi_utils.ok_dialog(heading='Error', text=text)
		return kodi_utils.ok_dialog(heading='Correcto', text='El token de acceso de lectura de listas es válido.')
	except Exception as e:
		return kodi_utils.ok_dialog(heading='Error', text='El token de acceso de lectura de listas ha fallado.[CR]%s' % str(e))
...
def external_scraper_choice(params):
	...
	if success:
		try:
			...
			kodi_utils.ok_dialog(text='Correcto.[CR][B]%s[/B] configurado como Raspador Externo' % module_name)
		except: kodi_utils.ok_dialog(text='Error')
	else:
		kodi_utils.ok_dialog(text='El Módulo [B]%s[/B] no es compatible.[CR]Por favor, elige un Módulo diferente...' % module_name.upper())
...
def playback_choice(params):
	...
	items = [{'line': 'Seleccionar Fuente', 'function': 'scrape'},
			{'line': 'Reexplorar y Seleccionar Fuente', 'function': 'clear_and_rescrape'}]
	if debrid_cache_check_available():
		items.append({'line': 'Reexplorar con Comprobación de Caché Externa [B]%s[/B]' % check_cache_status, 'function': 'rescrape_external_cache_check'})
	items.extend([{'line': 'Borrar Caché de Debrid y Mostrar Resultados', 'function': 'clear_debrid_cache_and_show'},
				{'line': 'Explorar con Todos los Scrapers Externos', 'function': 'scrape_with_disabled'},
				{'line': 'Explorar Ignorando Todos los Filtros', 'function': 'scrape_with_filters_ignored'}])
	if media_type == 'episode':
		items.append({'line': 'Explorar con un Grupo de Episodios Personalizado', 'function': 'scrape_with_episode_group'})
	if aliases:
		items.append({'line': 'Explorar con un Alias', 'function': 'scrape_with_aliases'})
	items.append({'line': 'Explorar con Valores Personalizados', 'function': 'scrape_with_custom_values'})
	...
	if choice == 'scrape':
		...
		if media_type == 'movie':
			play_params = {'mode': play_mode, 'media_type': 'movie', 'tmdb_id': meta['tmdb_id'],
		...
		if media_type == 'movie':
			...
			play_params = {'mode': play_mode, 'media_type': 'episode', 'tmdb_id': meta['tmdb_id'], 'season': season,
		...
		if not episode_details:
			kodi_utils.notification('No se encontró ningún episodio coincidente')
...
def set_quality_choice(params):
	...
	dl = ['Incluye 4K', 'Incluye 1080p', 'Incluye 720p', 'Incluye SD']
	fl = ['4K', '1080p', '720p', 'SD']
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Seleccione las Calidades Incluidas', 'multi_choice': 'true', 'preselect': preselect}
...
def extras_buttons_choice(params):
	...
	if media_type == None:
		choices = [('Configurar Botones de [B]Película[/B]', 'movie'),
					('Configurar Botones de [B]Serie[/B]', 'tvshow'),
					('Restaurar los Botones de [B]Película[/B] Predeterminada', 'restore.movie'),
					('Restaurar los Botones de [B]Serie[/B] Predeterminada', 'restore.tvshow'),
					('Restaurar los Botones de [B]Película y Serie[/B] Predeterminada', 'restore.both')]
		list_items = [{'line1': i[0]} for i in choices]
		kwargs = {'items': json.dumps(list_items), 'heading': 'Seleccione el Tipo de Medio Para Configurar los Botones.', 'narrow_window': 'true'}
		choice = kodi_utils.select_dialog(choices, **kwargs)
		if choice == None:
...
def extras_ratings_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Calificaciones a Mostrar', 'opción_multiple': 'true', 'preseleccionar': preselect}
	selection = kodi_utils.select_dialog(choices, **kwargs)
	if selection == None: return
	if selection == []:
		kodi_utils.ok_dialog(text='Debe Seleccionar al Menos 1 Proveedor de Calificaciones.')
...
def set_language_filter_choice(params):
	...
	kwargs = {'items': json.dumps(list_items), 'opción_multiple': multi_choice, 'preseleccionar': preselect}
	choice = kodi_utils.select_dialog(fl, **kwargs)
	if choice == None: return
...
def results_format_choice(params):
	choices = [('Lista', kodi_utils.get_icon('results_list', 'results')), ('Filas', kodi_utils.get_icon('results_row', 'results')),
				('Lista Amplia', kodi_utils.get_icon('results_widelist', 'results'))]
	list_items = [{'line1': item[0], 'icon': item[1]} for item in choices]
	kwargs = {'items': json.dumps(list_items), 'heading': 'Seleccione El Formato de Resultados'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice is None: return
...
def lists_cache_duration_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Establecer Duración de Cache de Lista Genérica', 'ventana_estrecha': 'true'}
...
def options_menu_choice(params, meta=None):
	...
	if menu_type in ('movie', 'episode') or menu_type in single_ep_list:
		base_str1, base_str2, on_str, off_str = '%s%s', 'Actualmente: [B]%s[/B]', 'Activado', 'Desactivado'
