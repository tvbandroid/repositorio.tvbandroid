...
def context_menu_items():
	...
	{'name': 'Explorar Colección', 'value': 'browse_movie_set'}, {'name': 'Explorar Temporadas', 'value': 'browse_seasons'},
	{'name': 'Explorar Episodios', 'value': 'browse_episodes'}, {'name': 'Explorar Recomendadas', 'value': 'recommended'}, {'name': 'Explorar Relacionadas', 'value': 'related'},
	{'name': 'Explorar Más Como Esta', 'value': 'more_like_this'}, {'name': 'Explorar Similares', 'value': 'similar'}, {'name': 'En Listas de Trakt', 'value': 'in_trakt_list'},
	{'name': 'Administrador de MDBList', 'value': 'mdblist_manager'}, {'name': 'Administrador de Listas Simkl', 'value': 'simkl_manager'}, {'name': 'Administrador de Listas Trakt', 'value': 'trakt_manager'}, {'name': 'Administrador de Listas TMDb', 'value': 'tmdb_manager'},
	{'name': 'Administrador de Listas Personales', 'value': 'personal_manager'}, {'name': 'Administrador de Favoritos', 'value': 'favorites_manager'}, {'name': 'Marcar como Visto/No Visto', 'value': 'mark_watched'},
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.playtvban').getAddonInfo(info)

def addon_version():
	return get_property('playtvban.addon_version') or addon_info('version')

def addon_path():
	return get_property('playtvban.addon_path') or addon_info('path')

def addon_profile():
	return get_property('playtvban.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('playtvban.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('playtvban.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return (
		get_property('playtvban.addon_fanart')
...
def resolve_list_icon(icon, default_name='folder'):
	...
	if icon_norm.startswith('special://') or 'plugin.video.playtvban/resources/media/' in icon_norm:
...
def sanitize_folder_url(url):
	if not url or 'plugin.video.playtvban' not in url: return url
	try:
...
def set_browse_exit_params(list_mode='tvshow', action=None):
	if external(): return
	set_property('playtvban.exit_params', browse_list_exit_params(list_mode, action))
...
def _resolve_view_id(view_type):
	try:
		...
		view_id = get_property('playtvban.%s' % view_type) or get_setting('playtvban.%s' % view_type) or get_setting(view_type)
	except: view_id = None
...
def set_view_mode(view_type, content='files', is_external=None):
	if get_property('playtvban.use_viewtypes') != 'true': return
...
def addon(addon_id='plugin.video.playtvban'):
...
def run_addon(addon='plugin.video.playtvban', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'playtvban' not in get_infolabel('Container.PluginName')
...
SHUTTING_DOWN_PROP = 'playtvban.shutting_down'
...
def disable_enable_addon(addon_name='plugin.video.playtvban'):
	import json
	try:
...
def addon_ui_busy():
	...
	if get_property('playtvban.window_loaded') == 'true': return True
	try:
		if xbmc.getCondVisibility('Window.IsActive(dialog)'): return True
	except: pass
	return False

def language_invoker_from_addon_xml(addon_name='plugin.video.playtvban'):
	try:
...
_ADDON_XML_SYNC_VERSION = 'playtvban.addon_xml_sync_version'
_ADDON_XML_APPLIED = 'playtvban.addon_xml_applied'
...
def sync_addon_xml_from_settings(addon_name='plugin.video.playtvban'):
	from xml.dom.minidom import parse as mdParse
	from caches.settings_cache import get_setting
	addon_xml = translate_path('special://home/addons/%s/addon.xml' % addon_name)
	if not path_exists(addon_xml): return False, False
	invoker_setting = get_setting('playtvban.reuse_language_invoker', None)
	icon_setting = get_setting('playtvban.addon_icon_choice', None)
...
def addon_xml_settings_diff(addon_name='plugin.video.playtvban'):
	from xml.dom.minidom import parse as mdParse
	from caches.settings_cache import get_setting
	addon_xml = translate_path('special://home/addons/%s/addon.xml' % addon_name)
...
def restart_addon_for_addon_xml_change(notify=True):
	if notify:
		notification('Se ha actualizado el addon.xml. Reiniciando Play TVBan...', 8000)
...
def update_kodi_addons_db(addon_name='plugin.video.playtvban'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('playtvban.external_scraper.module')
...
def progress_dialog(heading='', icon=None):
	...
	for _ in range(40):
		try:
			if progress_dialog.getProperty('playtvban.progress_ready') == 'true':
				break
		except: pass
		sleep(50)
...
def confirm_dialog(heading='', text='Estas seguro?', ok_label='OK', cancel_label='Cancelar', default_control=11, scroll=False):
	from windows.base_window import open_window
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('playtvban.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('playtvban.playback.volumecheck_percent') or '50'), 100)
