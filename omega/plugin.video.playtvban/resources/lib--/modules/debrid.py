...
class ExternalPackSource:
	...
	def browse_packs(self, download=False):
		...
		if not api:
			hide_busy_dialog()
			notification('Proveedor no compatible para la descarga del paquete: %s' % (provider or 'Unknown'))
			return None
		show_busy_dialog()
		...
		if not pack_choices:
			if provider == 'TorBox':
				notification('TorBox: Aún no hay archivo(s) de video. Inténtalo de nuevo en un momento.', 4500)
			elif download:
	...
	if unchecked_hashes:
		...
		try:
			...
			if err in ('NO_SERVER', 'MAGNET_NO_SERVER'):
				...
				set_property('playtvban.debrid_cache_api_error', str(err))
				...
				if response.get('error'):
					...
					if err_code in ('NO_SERVER', 'MAGNET_NO_SERVER'):
						try:
							...
							set_property('playtvban.debrid_cache_api_error', err_code)
						except: pass
						break
					continue
		...
		if not api_responded:
			...
			if 'AllDebrid' in active_debrid and not get_property('playtvban.debrid_cache_api_error'):
				try:
			...
			if not cached_results:
				if not get_property('playtvban.debrid_cache_api_error'):
				...
				try:
					...
					if api_errors and all(e in ('NO_SERVER', 'MAGNET_NO_SERVER') for e in api_errors):
						set_property('playtvban.debrid_cache_api_error', 'NO_SERVER')
					elif not get_property('playtvban.debrid_cache_api_error'):
						clear_property('playtvban.debrid_cache_api_error')
				except: pass
				return cached_hashes
			try:
