...
def runner(params):
	...
	if action == 'image':
		...
		kodi_utils.notification('Se ha Iniciado la Descarga del Paquete de Archivos Múltiples...', 3500, image)
...
def download_threads_manager(multi_downloads, image, pack_cleanup=None):
	...
	for item in multi_downloads:
		...
		kodi_utils.set_property('playtvban.active_queued_downloads', json.dumps(remaining_downloads))
	kodi_utils.clear_property('playtvban.active_queued_downloads')
	if pack_cleanup:
...
class Downloader:
	...
	def get_active_downloads(self):
		return json.loads(kodi_utils.get_property('playtvban.active_downloads') or '[]')
	...
	def add_active_download(self):
		...
		kodi_utils.set_property('playtvban.active_downloads', json.dumps(active_downloads))
	...
	def remove_active_download(self):
		...
		if active_downloads: kodi_utils.set_property('playtvban.active_downloads', json.dumps(active_downloads))
		else: self.clear_active_downloads()

	def clear_active_downloads(self):
		kodi_utils.clear_property('playtvban.active_downloads')

	def set_percent_property(self, percent):
		kodi_utils.set_property('playtvban.%s' % self.final_name, str(percent))

	def check_status(self):
		status = kodi_utils.get_property('playtvban.download_status.%s' % self.final_name)
		if status in ('unpaused', 'cancelled'): kodi_utils.clear_property('playtvban.download_status.%s' % self.final_name)
		return status
		...
		if url in (None, 'None', ''):
			if self.action == 'meta.single':
				...
				if not link:
					...
					if url:
						...
						return self.return_notification(_notification='TorBox: El enlace de descarga no está listo. Espere a que el torrent finalice en TorBox y vuelva a intentar descargar el paquete.')
				elif self.provider in ('offcloud', 'Offcloud'):
					# display_magnet_pack / cache_download ya devuelve URLs directas de Offcloud CDN
					url = link
				else:
					url = None
		else:
			if self.action.startswith('cloud'):
				if '_direct' in self.action:
					...
					if not url:
						return self.return_notification(_notification='TorBox: No se pudo resolver el enlace de descarga.')
					url = api.add_headers_to_url(url)
	...
	def start_download(self):
		if self.action == 'meta.pack':
			kodi_utils.notification('Se ha iniciado la descarga del paquete: %s' % self.final_name.replace('.', ' ').replace('_', ' '), 3000, self.image)
		...
		while True:
			...
			try:        
				...
				if hasattr(e, 'errno'):
					errno = e.errno
				if errno == 10035: # 'No se pudo completar de inmediato una operación de socket no bloqueante'
					pass
				if errno == 10054: #'Una conexión existente fue cerrada forzosamente por el host remoto'
					errors = 10 #forzar reanudación
					sleep_time = 30
				if errno == 11001: # 'getaddrinfo falló'
					errors = 10 #forzar reanudación
					sleep_time = 30
			if chunk:
				errors = 0
	...
	def confirm_download(self):
		...
		if getattr(self, 'content_unknown', False):
			text = 'No se pudo determinar el tamaño del archivo desde el servidor.[CR]Continuar con la descarga?'
		else:
			text = 'El archivo completo ocupa [B]%d MB[/B][CR]Continuar con la descarga?' % self.mb
