...
def _pick_import_file(default_dir):
	path = kodi_utils.browse_file(BACKUP_MASK, default_dir, heading='Seleccionar copia de seguridad de favoritos e historial', force_defaultt=True)
	if not path:
		return None
	...
	if not tpath.lower().endswith('.json'):
		kodi_utils.ok_dialog(heading='Importar favoritos e historial', text='Seleccione una copia de seguridad de Play TVBan. (.json).')
		return None
	return tpath
...
def export_data(params):
	...
	if not payload:
		return kodi_utils.ok_dialog(heading='Exportar favoritos e historial', text='No se encontraron favoritos ni historial para exportar en este dispositivo.')
	folder = pick_export_folder(heading='Seleccionar carpeta de exportación')
	if not folder:
		return
	filename = 'playtvban-backup-%s.json' % datetime.now().strftime('%Y%m%d-%H%M%S')
	path = os.path.join(folder, filename)
	preview = _export_preview_text(filename, counts)
	if not kodi_utils.confirm_dialog(heading='Exportar favoritos e historial', text=preview, ok_label='Exportar', cancel_label='Cancelar', default_control=10, scroll=True):
		return
	try:
