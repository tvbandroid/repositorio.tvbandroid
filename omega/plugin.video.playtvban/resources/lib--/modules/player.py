...
PROP_NEXTEP_PENDING = 'playtvban.nextep_pending'
PROP_NEXTEP_PREP_SCHEDULED = 'playtvban.nextep_prep_scheduled'
...
class PlayTVBanPlayer(xbmc.Player):
	def __init__ (self):
	...
	def check_playback_start_generic(self):
		...
		while self.playback_successful is None:
			...
			if self.kodi_monitor.abortRequested():
				...
				try:
					if ku.get_property('playtvban.browse_playback') == 'true':
					...
					if self.media_type == 'episode':
						if self.autoplay_nextep or self.autoscrape_nextep:
							...
							try:
								...
								if _nextep_remaining > 0: ku.set_property('playtvban.nextep_remaining', str(_nextep_remaining))
							except: pass
	...
	def make_listing(self):
		...
		if self.is_generic:
			...
			play_name = ku.get_property('playtvban.tb.play_filename') or self.url
			info_tag.setFilenameAndPath(play_name)
			info_tag.setTitle(os.path.basename(play_name) if play_name else '')
			mime = ku.get_property('playtvban.tb.play_mime')
			if not mime:
	...
	def _log_nextep(self, message):
		try: ku.logger('Play TVBan', message)
		except: pass
	...
	def _log_intro_skip(self, message):
		try: ku.logger('Play TVBan', '%s (play_type=%s)' % (message, self._intro_skip_play_type_label()))
		except: pass
	...
	def _should_prep_next_ep(self):
		...
		if ku.get_property(PROP_NEXTEP_PREP_SCHEDULED) == 'true':
			return False
		if self.autoplay_nextep and ku.get_property('playtvban.nextep_scrape_ready') == 'true':
			try:
	...
	def _schedule_next_ep(self):
		...
		def _work():
			try:
				...
				ku.logger('Play TVBan', 'Next episode prep failed: %s' % exc)
			finally:
	...
	def info_next_ep(self):
		...
		credits_log = ' entrada_créditos=%ss' % credits_entry if credits_entry is not None else ''
		outro_start = self.nextep_settings.get('outro_start')
		outro_log = ' inicio_outro=%.1fs' % outro_start if outro_start is not None else ''
		self._log_nextep('Temporización del Siguiente Episodio: tipo_reproducción=%s alerta=%s origen=%s mostrar_en=%ss preparación=%ss inicio_preparación=%ss total=%ss%s%s' % (
	...
	def _execute_intro_skip_seek(self, start_sec, end_sec, source):
		...
		try:
			...
			if abs(actual - end_sec) > 8:
				if not self.seek(end_sec, False):
										self._log_intro_skip('Omisión de introducción fallida: el salto no se aplicó (en %.1fs, se esperaba %.1fs)' % (actual, end_sec))
					return False
				ku.sleep(400)
				actual = float(self.getTime())
				if abs(actual - end_sec) > 8:
			...
			self._log_intro_skip('Omisión de introducción fallida: verificación del salto (%s)' % exc)
			return False
		self._log_intro_skip('Omisión de introducción (%s): %.1fs -> %.1fs' % (source, start_sec, end_sec))
		return True
	...
	def clear_playback_properties(self, clear_navigation=True):
		if clear_navigation:
			ku.clear_property('playtvban.window_stack')
