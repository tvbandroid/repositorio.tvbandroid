...
def export_settings(params):
	...
	if not folder:
		return
	filename = 'playtvban-settings-%s.zip' % datetime.now().strftime('%Y%m%d-%H%M%S')
...
def import_settings(params):
	...
	if not inventory['databases']:
		return kodi_utils.ok_dialog(heading='Import settings', text='That zip does not contain Play TVBan settings.')
...
def _read_manifest(archive):
	...
	if manifest.get('type') != 'playtvban_settings':
		raise ValueError('This zip is not a Play TVBan settings backup.')
...
def _import_confirm_text(path, manifest, inventory):
	lines = [
		...
		'From Play TVBan %s (%s)' % (manifest.get('addon_version') or 'unknown', (manifest.get('exported') or 'unknown')[:10]),
		'',
		'This replaces your current Play TVBan settings, menus, and lists.',
...
def _apply_settings_import(zip_path, inventory):
	profile = kodi_utils.translate_path(kodi_utils.addon_profile())
	temp_dir = os.path.join(kodi_utils.translate_path('special://temp'), 'playtvban_settings_import')
