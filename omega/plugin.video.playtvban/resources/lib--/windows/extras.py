...
class Extras(BaseDialog):
	...
	def make_ratings(self, win_prop=4000):
		...
		for item in final_ratings:
			...
			self.set_image(win_prop + 100 + item['_id'], 'playtvban_flags/ratings/%s' % item['icon'])
	...
	def assign_buttons(self):
		setting_id_base = 'playtvban.extras.%s.button' % self.media_type
		for item in self.button_ids[:-1]:
			...
			try:
				...
				self.restore_setting_default({'setting_id': setting_id.replace('playtvban.', ''), 'silent': 'true'})
				button_action = self.get_setting(setting_id)
				button_label = self.button_label_values[self.media_type][button_action]
	...
	def set_infoline1(self, rating_data=None, remove_rating=False):
		if remove_rating:
			...
			rating, image = data['rating'], 'playtvban_flags/ratings/%s' % data['icon']
