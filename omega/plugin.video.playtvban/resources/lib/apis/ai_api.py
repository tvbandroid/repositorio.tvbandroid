...
def get_currently_active_model():
	...
	try:
		...
		if timeout_models:
			# timeout_ends == 0 significa "fallo temporal / omitir en esta cadena de intento" — no
			# tratarlo como expirado (0 <= now siempre era verdadero y provocaba reintentos infinitos del mismo modelo).
...
def set_currently_active_models(model_id, status_code):
	try:
		...
		# 401/403 = clave no válida; 429 = cuota excedida — desactivar inmediatamente con una notificación.
		if status_code in (401, 403, 429) or current_fails >= 2:
			fails, timeout_ends = 0, get_timestamp(24)
			notification('Desactivando %s: Código de error %s' % (model_id, status_code))
		else:
			# Fallo temporal: omitir este modelo durante un periodo corto y después probar el siguiente.
...
def ai_similar_call(media_type, tmdb_id, meta, limit, timeout=30):
	...
	if not model_id:
		logger('Play TVBan', 'IA similar: no hay ningún modelo disponible (todos desactivados o sin clave API)')
		return None
	...
	try:
		...
		logger('Play TVBan', 'IA similar: error en la solicitud de %s: %s' % (model_id, e))
		if set_currently_active_models(model_id, 0): return ai_similar_call(media_type, tmdb_id, meta, limit, timeout)
		return None
	status_code = response.status_code
	if status_code != 200:
		logger('Play TVBan', 'IA similar: %s HTTP %s' % (model_id, status_code))
	...
	if not result:
		logger('Play TVBan', 'IA similar: %s no devolvió recomendaciones analizables' % model_id)
		return None
	return result