...
Does not sync watched ticks, resume, or lists into Play TVBan. Prefer
Watched Indicators = Play TVBan (or another provider) for UI state.
"""
import json
import time
import requests
...
APP_UA = 'PlayTVBan-WeTrakr/%s' % kodi_utils.addon_version()

WETRAKR_SCROBBLE_ONLY_TEXT = (
	'[B]WeTrakr solo hace scrobble en Play TVBan.[/B][CR][CR]'
	'Autoriza en Cuentas Meta, y luego usa [B]Activar Scrobbling[/B] para activar o desactivar '
	'el envío de eventos de reproducción/finalización sin revocar el acceso.[CR][CR]'
	'Cuando está activado, Play TVBan le indica a WeTrakr cuándo empiezas y terminas de ver, '
	'para que los títulos aparezcan en WeTrakr (Reproduciendo Ahora / historial).[CR][CR]'
	'[B]No[/B] trae de vuelta a Play TVBan las marcas de visto, puntos de reanudación, '
	'Próximos Episodios ni listas.[CR][CR]'
	'Mantén el [B]Proveedor de Estado de Visto[/B] en [B]Play TVBan[/B] (o MDBList / Simkl / Trakt) '
	'para las marcas y listas dentro del addon.'
)
...
def _token():
	...
	if token in (None, '0', '', 'empty_setting'):
		token = get_setting('playtvban.wetrakr.token', '0')
	return token
...
def wetrakr_scrobble_threshold():
	try: return max(50, min(99, int(get_setting('playtvban.wetrakr.scrobble_threshold', '90'))))
	except: return 80
...
def wetrakr_authenticate(dummy=''):
	if kodi_utils.addon_installed('script.wetrakr') and kodi_utils.addon_enabled('script.wetrakr'):
		...
		if ext_token:
			kodi_utils.ok_dialog(heading='WeTrakr',
				text='El add-on oficial de [B]WeTrakr[/B] para Kodi ya está autorizado.[CR][CR]'
				'Play TVBan dejará el scrobbling a ese add-on para que los eventos no se envíen dos veces.[CR][CR]'
				'Revoca o desactiva [B]script.wetrakr[/B] si quieres que Play TVBan haga el scrobbling en su lugar.')
			return False
	icon = _wetrakr_icon()
	code_data = request_device_code()
	if not code_data or not code_data.get('device_code'):
		return kodi_utils.notification('Autorización de WeTrakr Fallida', 3000, icon)
	...
	content = (
		'Introduce [B]%s[/B] en [B]%s[/B][CR]O escanea el [B]Código QR[/B][CR]'
		'Enlace copiado al portapapeles[CR][CR]Esperando autorización...'
		% (user_code, verification_url.replace('https://', '')))
	progress = kodi_utils.progress_dialog('Autorizar WeTrakr', qr_code)
	...
	while time.time() < expires:
		if progress.iscanceled():
			progress.close()
			return kodi_utils.notification('Autorización de WeTrakr Cancelada', 3000, icon)
		data = _poll_device_token(device_code)
		if data:
			if data.get('access_token'):
				token = data['access_token']
				username = data.get('username') or 'Usuario de WeTrakr'
				break
	...
	kodi_utils.notification('Cuenta de WeTrakr Autorizada', 3000, icon)
...
def wetrakr_revoke_authentication(dummy=''):
	...
	kodi_utils.notification('Autorización de WeTrakr Restablecida', 3000, _wetrakr_icon())
	try: kodi_utils.container_refresh()
	except: pass
	return True

def wetrakr_about(dummy=''):
	kodi_utils.ok_dialog(heading='WeTrakr (Solo Scrobble)', text=WETRAKR_SCROBBLE_ONLY_TEXT, scroll=True)
	return True