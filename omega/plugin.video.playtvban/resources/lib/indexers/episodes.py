	...
	def _process():
		for item in episodes_data:
			try:
				...
				set_properties({
					'episode_type': episode_type, 'playtvban.extras_params': extras_params, 'playtvban.options_params': options_params,
...
def build_single_episode(list_type, params={}):
	...
	def _process(_position, ep_data):
		try:
			...
			cm_append(['extras', ('[B]Extras[/B]', 'RunPlugin(%s)' % extras_params)])
			cm_append(['options', ('[B]Opciones[/B]', 'RunPlugin(%s)' % options_params)])
			cm_append(['playback_options', ('[B]Opciones de Reproducción[/B]', 'RunPlugin(%s)' % \
			...
			if not unaired:
				if playcount:
					cm_append(['mark_watched', ('[B]Marcar No Visto[/B]', 'RunPlugin(%s)' % build_url({'mode': 'watched_status.mark_episode', 'action': 'mark_as_unwatched',
											...
											'tmdb_id': tmdb_id, 'tvdb_id': tvdb_id, 'season': season, 'episode': episode,  'title': title}))])
				if progress:
					cm_append(['mark_watched', ('[B]Borrar Progreso[/B]', 'RunPlugin(%s)' % \
								build_url({'mode': 'watched_status.erase_bookmark', 'media_type': 'episode', 'tmdb_id': tmdb_id,
											'season': season, 'episode': episode, 'refresh': 'true'}))])
				if unwatched_info:
