...
def oc_account_info():
	try:
		...
		body = []
		append = body.append
		append('[B]Correo electrónico[/B]: %s' % account_info.get('email', ''))
		append('[B]ID de usuario[/B]: %s' % account_info.get('user_id') or account_info.get('userId', ''))
		...
		if 'can_download' in account_info:
			append('[B]Puede Descargar[/B]: %s' % account_info.get('can_download'))
		...
		append('[B]Límite de la Nube[/B]: %s' % cloud_limit)
		append('[I]Puede Descargar y Límite de la Nube están establecidos por su plan de Offcloud (offcloud.com), no en Play TVBan.[/I]')
		return kodi_utils.show_text('OFFCLOUD', '\n\n'.join(body), font_size='large')
	except Exception:
		kodi_utils.hide_busy_dialog()
		return kodi_utils.ok_dialog(text='Offcloud: No se pudo cargar la información de la cuenta')