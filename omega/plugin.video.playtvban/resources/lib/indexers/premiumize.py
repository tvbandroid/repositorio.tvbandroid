	...
	def _builder():
		for count, item in enumerate(cloud_files, 1):
			try:
				...
				if file_type == 'folder':
					is_folder = True
					display = '%02d | [B]CARPETA[/B] | [I]%s [/I]' % (count, name)
				...
				cm_append(('[B]Renombrar %s[/B]' % file_type.capitalize(),'RunPlugin(%s)' % kodi_utils.build_url(rename_params)))
				cm_append(('[B]Eliminar %s[/B]' % file_type.capitalize(),'RunPlugin(%s)' % kodi_utils.build_url(delete_params)))
...
def pm_transfers():
		...
		for count, item in enumerate(transfer_files, 1):
			try:
				...
				if _is_folder_transfer(item):
					...
					display = '%02d | %d%% | [B]CARPETA[/B] | [I]%s [/I]' % (count, progress, name)
					if is_folder:
					...
					if ready and file_id:
						...
						if isinstance(details, dict) and details.get('link'):
							...
							display_size = float(int(size)) / 1073741824
							display = '%02d | %d%% | [B]ARCHIVO[/B] | %.2f GB | [I]%s [/I]' % (count, progress, display_size, name)
							url_params = {'mode': 'playback.video', 'url': url_link, 'obj': 'video'}
							down_file_params = {'mode': 'downloader.runner', 'name': item.get('name', ''), 'url': url_link, 'action': 'cloud.premiumize', 'image': icon}
							cm_append(('[B]Descargar Archivo[/B]', 'RunPlugin(%s)' % kodi_utils.build_url(down_file_params)))
						else:
							display = '%02d | %d%% | [B]ARCHIVO[/B] | [I]%s [/I]' % (count, progress, name)
					else:
						display = '%02d | %d%% | [B]ARCHIVO[/B] | [I]%s [/I]' % (count, progress, name)
				url = kodi_utils.build_url(url_params)
				listitem = kodi_utils.make_listitem()
	...
	try:
		...
		if not result:
			api_error = 'No se pudo cargar el historial'
		elif str(result.get('status', '')).lower() != 'success':
			api_error = result.get('message') or 'No se pudo cargar el historial'
		else:
	...
	if not transfer_files:
		kodi_utils.notification('Premiumize: No hay transferencias en el historial', 2500)
	elif not items:
		kodi_utils.notification('Premiumize: Se encontraron transferencias, pero no se pudieron mostrar', 3500)
	handle = int(sys.argv[1])
...
def pm_rename(file_type, file_id, current_name):
	new_name = kodi_utils.kodi_dialog().input('PlayTVBan', defaultt=current_name)
...
def pm_account_info():
	try:
		...
		body = []
		append = body.append
		append('[B]ID de Cliente:[/B] %s' % customer_id)
		append_expiry_lines(body, fetch_expiry_summary('pm'))
		append('[B]Puntos Utilizados:[/B] %.f' % points_used)
		append('[B]Espacio Utilizado:[/B] %.2f' % space_used)
		append('[B]Uso Justo (Porcentaje Utilizado):[/B] %s%%' % percentage_used)
		kodi_utils.hide_busy_dialog()
