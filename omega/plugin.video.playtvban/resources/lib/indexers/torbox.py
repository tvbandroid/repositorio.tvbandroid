...
def _tb_play_cloud(resolved_link, filename=''):
	...
	try:
		if filename:
			kodi_utils.set_property('playtvban.tb.play_mime', _tb_mime_type(filename))
			kodi_utils.set_property('playtvban.tb.play_filename', filename)
		from modules.player import RedLightPlayer
		RedLightPlayer().run(resolved_link, 'video')
	finally:
		kodi_utils.clear_property('playtvban.tb.play_mime')
		kodi_utils.clear_property('playtvban.tb.play_filename')
	return None
