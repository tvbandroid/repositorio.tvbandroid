	...
	def _builder():
		for item in lists:
			try:
				...
				('[B]No me gusta la Lista[/B]', 'RunPlugin(%s)' % build_url({'mode': 'trakt.trakt_unlike_a_list', 'user': user, 'list_slug': slug, 'list_id': list_id})),
	...
	try:
		...
		if total_pages > 2 and jump_to_enabled() and not is_external:
				kodi_utils.add_dir(handle, {'mode': 'navigate_to_page_choice', 'current_page': page_no, 'total_pages': total_pages, 'url_params': json.dumps(new_params)},
											'Ir a...', 'item_jump', kodi_utils.get_icon('item_jump_landscape'), isFolder=False)
