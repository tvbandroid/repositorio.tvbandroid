...
def clear_local_bookmarks():
	try:
		...
		file_ids = dbcon.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.playtvban%'").fetchall()
...
def get_in_progress_movies(dummy_arg, page_no):
	...
	logger('Play TVBan', 'get_in_progress_movies: %s item(s) from %s' % (len(data), source))
...
def get_in_progress_tvshows(dummy_arg, page_no):
	...
	logger('Play TVBan', 'get_in_progress_tvshows: %s item(s) from %s' % (len(results), source))
...
def get_in_progress_episodes():
	...
	logger('Play TVBan', 'get_in_progress_episodes: %s item(s) from %s' % (len(episode_list), source))
