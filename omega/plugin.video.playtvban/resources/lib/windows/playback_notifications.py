...
class NextEpisode(BaseDialog):
	...
	'mid_season_premiere': ('Estreno de Mitad de Temporada', 'b385b503'),
	'series_finale': ('Final de la Serie', 'b38503b5'),
	'season_finale': ('Final de Temporada', 'b3b50385'),
	'mid_season_finale': ('Final de Mitad de Temporada', 'b3b58503'),
	'':  (None, None)}
	...
	def monitor(self):
		...
		try:
			while not self.closed and pause_timer >= 0:
				...
				self.setProperty('pause_timer', '%02d %s' % (pause_timer, 'segundos' if pause_timer > 1 else 'segundo'))
				self.sleep(1000)
	...
	def make_stingers(self):
		for k, v in self.stinger_dict.items():
			if k in self.stingers:
				...
				self.set_image(v['id'], 'playtvban_common/overlay_selected.png')
			else:
				self.setProperty(v['property'], 'red')
				self.set_image(v['id'], 'playtvban_common/cross.png')
