...
class SourcesResults(BaseDialog):
	def __init__(self, *args, **kwargs):
		...
		self.tint_focused_background = get_setting('playtvban.highlight.tint_focused_background') == 'true'
		self.highlight_alpha = get_setting('playtvban.highlight.background_opacity', '66')
		self.make_items()
...
class SourcesPlayback(BaseDialog):
	...
	def onInit(self):
		...
		set_property('playtvban.scrape.ready', 'true')
		self.setProperty('playtvban.scrape.ready', 'true')
	...
	def enable_resolver(self):
		...
		set_property('playtvban.scrape.percent', '0')
	...
	def update_resolver(self, text='', percent=0):
		...
		pct = int(percent)
		try:
			set_property('playtvban.scrape.percent', str(pct))
			self.setProperty('percent', str(pct))
		except: pass
