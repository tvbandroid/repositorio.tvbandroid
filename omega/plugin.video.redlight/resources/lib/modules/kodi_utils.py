...
def browse_directory(defaultt='', heading='Seleccionar carpeta', use_defaultt=False, confirm_unchanged=False, force_defaultt=False):
...
def browse_file(mask='', defaultt='', heading='Seleccionar archivo', force_defaultt=False):
...
def ok_dialog(heading='', text='No hay resultados', ok_label='OK', scroll=False):
	from windows.base_window import open_window
	needs_scroll = scroll and _dialog_needs_scroll(text)
...
def show_text(heading, text=None, file=None, font_size='small', kodi_log=False):
	...
	if kodi_log:
		confirm = confirm_dialog(text='Mostrar solo los errores del registro?', ok_label='Yes', cancel_label='No')
...
LIST_ITEM_NOT_IN_LIST = 'Artículo no incluido en la lista.'
...
def player_check(mode, params):
	...
	else: ok_dialog('Se Ha Detectado Reproducción Externa', 'la reproducción a través de los addons externos no es compatible.')
...
def upload_logfile(params):
	...
	try:
		...
		if 'key' not in response:
			return ok_dialog(text='Error. Falló la Carga del Registro.')
		user_code = response['key']
		url = '%s%s' % (url, user_code)
		copy2clip(url)
		qr_code = make_qrcode(url) or ''
		progressDialog = progress_dialog(heading='Cargador de Registros de Kodi', icon=qr_code)
		countdown_secs = 120
		remaining = countdown_secs
		while not progressDialog.iscanceled() and remaining > 0:
			progressDialog.update(
				'Comparta o Acceda con esta URL: [B]%s[/B][CR]O escanee el código QR en otro dispositivo.[CR][CR]Se cierra automáticamente en [B]%d[/B] segundos (Volver para cerrar ahora).' % (url, remaining),
		...
		ok_dialog(text='Error. Falló la Carga del Registro.')
	finally:
		hide_busy_dialog()
		if progressDialog:
