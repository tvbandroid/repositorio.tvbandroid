...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    ...
    msgupdate = '[COLOR gold]Update available for installed build: [/COLOR]'
    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)
    current_version = '[COLOR gold]Current Version:[/COLOR] [COLOR red]v[/COLOR][COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)
    latest_version = '[COLOR gold]Latest Version:[/COLOR] [COLOR red]v[/COLOR][COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
    ...
    install = xbmcgui.Dialog().yesno(CONFIG.ADDONTITLE, final_msg,
                                     yeslabel='[B][COLOR springgreen]Install[/COLOR][/B]', nolabel='[B][COLOR red]Ignore[/COLOR][/B]')
    if install:
