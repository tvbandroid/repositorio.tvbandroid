    ...
    if who == 'all':
        for log in ORDER:
            if os.path.exists(DEBRIDID[log]['path']):
                ...
                logging.log('[Información de Debrid] {0}({1}) no está instalado'.format(DEBRIDID[log]['name'], DEBRIDID[log]['plugin']), level=xbmc.LOGERROR)
        ...
        if DEBRIDID[who]:
            ...
            logging.log('[Información de Debrid] Entrada Inválida: {0}'.format(who), level=xbmc.LOGERROR)
    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, DEBRIDID[who]['name']),
                               '[COLOR {0}]Información de Debrid: Eliminado![/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               DEBRIDID[who]['icon'])
    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Información de Debrid Guardados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Información de Debrid] No se Puede Actualizar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Información de Debrid No Registrados para {0}'.format(name))
    elif do == 'restore':
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Información de Debrid Restaurados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Información de Debrid] No se Puede Restaurar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Información de Debrid No Encontrados para {0}'.format(name))
    elif do == 'clearaddon':
        ...
        if os.path.exists(settings):
            try:
                ...
                logging.log("[Información de Debrid] No se Puede Borrar el Addon {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
    xbmc.executebuiltin('Container.Refresh()')
        ...
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Desea guardar la [COLOR {0}]Información de Debrid[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"[COLOR khakiAddon: [COLOR springgreen][B]{0}[/B][/COLOR]".format(u)
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(DEBRIDID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]Información de Debrid: Importada![/COLOR]'.format(CONFIG.COLOR2))
    ...
    if DEBRIDID[who]:
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE,
                          '{0} no está instalado actualmente.'.format(DEBRIDID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
