...
class AdvancedMenu:
    ...
    def show_menu(self, url=None):
        directory.add_dir('[COLOR gold]Quick Configure advancedsettings.xml[/COLOR]',
                               {'mode': 'advanced_settings', 'action': 'quick_configure'}, icon=CONFIG.ICONMAINT,
                               themeit=CONFIG.THEME3)

        if os.path.exists(CONFIG.ADVANCED):
            directory.add_file('[COLOR gold]View Current advancedsettings.xml[/COLOR]',
                               {'mode': 'advanced_settings', 'action': 'view_current'}, icon=CONFIG.ICONMAINT,
                               themeit=CONFIG.THEME3)
            directory.add_file('[COLOR gold]Remove Current advancedsettings.xml[/COLOR]',
    ...
    def quick_configure(self):
        directory.add_file('[COLOR gold]Changes will not be reflected until Kodi is restarted![/COLOR]', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
        directory.add_file('[COLOR gold]Click here to Force Close Kodi![/COLOR]', {'mode': 'forceclose'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
        ...
        directory.add_dir('[COLOR gold]Network[/COLOR]', {'mode': 'advanced_settings', 'action': 'show_section', 'tags': 'cache|network'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
    ...
    def show_section(self, tags):
        ...
        if len(self.tags) == 0:
            directory.add_file('[COLOR gold]No settings for this category exist in your current advancedsettings.xml file[/COLOR]', icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
            directory.add_separator()
    ...
    def write_advanced(self, name, url):
        ...
        if response:
            if os.path.exists(CONFIG.ADVANCED):
                ...
                choice = self.dialog.yesno(CONFIG.ADDONTITLE,
                                           "[COLOR {0}]Would you like to write [COLOR {1}]{2}[/COLOR]?[/COLOR]".format(
                                               CONFIG.COLOR2, CONFIG.COLOR1, name),
