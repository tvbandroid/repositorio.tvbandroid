...
def cleanup_backup():
    ...
    if len(folder) == 0:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Back-up Location: Empty[/COLOR]".format(CONFIG.COLOR2))
        return
...
class Backup:
    ...
    def backup_addon_pack(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Would you like to create an Addon Pack?[/COLOR]".format(CONFIG.COLOR2),
                             nolabel="[B][COLOR red]Cancel Back-up[/COLOR][/B]",
            ...
            self.dialog.ok(CONFIG.ADDONTITLE,"[COLOR {0}]{1}[/COLOR] [COLOR {2}]back-up successful:[/COLOR]".format(CONFIG.COLOR1, name, CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, zipname))

    def backup_build(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Would you like to back-up the current build?[/COLOR]".format(CONFIG.COLOR2), nolabel="[B][COLOR red]Cancel Back-up[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Back-up Build[/COLOR][/B]"):
            if name == "":
            ...
            try:
                ...
                try:
                    ...
                    logging.log("Unable to create {0}.zip".format(name), level=xbmc.LOGERROR)
                    if self.dialog.yesno(CONFIG.ADDONTITLE,
                                         "[COLOR {0}]We are unable to write to the current back-up directory, would you like to change the location?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Change Directory[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
                        CONFIG.open_settings()
                        return
                ...
                for file in files:
                    try:
                        ...
                        try:
                            ...
                            logging.log("[Back Up] Type = build: Unable to back-up {0}".format(file))
                            logging.log("{0} / {1}".format(Exception, e))
                        if self.progress_dialog.iscanceled():
                            self.progress_dialog.close()
                            logging.log_notify(CONFIG.ADDONTITLE,
                                               "[COLOR {0}]Back-up Cancelled[/COLOR]".format(CONFIG.COLOR2))
                            sys.exit()
                    except Exception as e:
                        logging.log("[Back Up] Type = build: Unable to back-up {0}".format(file))
                        logging.log("Build Back-up Error: {0}".format(str(e)))
            ...
            if len(binaries) > 0:
                self.dialog.ok(CONFIG.ADDONTITLE,
                               "[COLOR {0}]The following add-ons were excluded from the back-up because they are platform specific:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, ', '.join(binaries)))
    ...
    def backup_gui(self, name=""):
        ...
        if os.path.exists(CONFIG.GUISETTINGS):
            try:
                ...
                try:
                    ...
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]We are unable to write to the current back-up directory, would you like to change the location?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Change Directory[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
                        CONFIG.open_settings()
                        return
        ...
        if name == "":
            self.dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]GUI Fix back-up successful:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, guizipname))

    def backup_theme(self, name=""):
        if not self.dialog.yesno(CONFIG.ADDONTITLE + ''.format(CONFIG.COLOR2), "[COLOR {0}]Would you like to create a theme back-up?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR springgreen]Back-up Theme[/COLOR][/B]",
                                 nolabel="[B][COLOR red]Cancel Back-up[/COLOR][/B]"):
            logging.log_notify("Theme Back-up", "Cancelled!")
            return False
        if name == "":
        ...
        try:
            if not CONFIG.SKIN not in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
                ...
                if len(match2) > 1:
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Back-up[/COLOR]'.format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]Would you like to go through the Texture Files for?[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                         yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                        for xbt in match2:
                            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Back-up[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Would you like to add the Texture File [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR1, CONFIG.COLOR2, xbt.replace(skinfold, "")[1:]) + '\n' + "from [COLOR {0}]{1}[/COLOR][/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                    yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                    nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                                fn = xbt
                    ...
                    for xbt in match2:
                        if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Back-up[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Would you like to add the Texture File [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, xbt.replace(skinfold, "")[1:]) + '\n' + "from [COLOR {0}]{1}[/COLOR][/COLOR]".format(CONFIG.COLOR1, CONFIG.SKIN),
                                yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]",
                                nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                            fn = xbt
                            fn2 = fn.replace(CONFIG.HOME, "")
                ...
                if os.path.exists(ad_skin):
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Back-up[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Would you like to go add the [COLOR {1}]settings.xml[/COLOR] in [COLOR {2}]/addon_data/[/COLOR] for?".format(
                ...
                if 'script.skinshortcuts' in match:
                    if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Back-up[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Would you like to go add the [COLOR {1}]settings.xml[/COLOR] for [COLOR {2}]script.skinshortcuts[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
                                         yeslabel="[B][COLOR springgreen]Add Settings[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Skip Settings[/COLOR][/B]"):
                    ...
                    for base, dirs, files in os.walk(fn):
                        ...
                        for file in files:
                            try:
                                ...
                                logging.log("[Back Up] Type = theme: Unable to back-up {0}".format(file))
                                logging.log("Back-up Error: {0}".format(str(e)))
    ...
    def backup_addon_data(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Would you like to back-up the current addon_data?[/COLOR]".format(
                                 CONFIG.COLOR2),
                             nolabel="[B][COLOR red]Cancel Back-up[/COLOR][/B]",
                             yeslabel="[B][COLOR springgreen]Yes, Back-up[/COLOR][/B]"):
            if name == "":
            ...
            try:
                ...
                try:
                    ...
                    if self.dialog.yesno(CONFIG.ADDONTITLE,
                                         "[COLOR {0}]We are unable to write to the current back-up directory, would you like to change the location?[/COLOR]".format(
                                             CONFIG.COLOR2),
                                         yeslabel="[B][COLOR springgreen]Change Directory[/COLOR][/B]",
                                         nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
                        CONFIG.open_settings()
                ...
                for file in files:
                    try:
                        ...
                        try:
                            ...
                            logging.log("[Back Up] Type = addon_data: Unable to back-up {0}".format(file))
                            logging.log("Back-up Error: {0}".format(str(e)))
                    except Exception as e:
                        logging.log("[Back Up] Type = addon_data: Unable to back-up {0}".format(file))
                        logging.log("Back-up Error: {0}".format(str(e)))
