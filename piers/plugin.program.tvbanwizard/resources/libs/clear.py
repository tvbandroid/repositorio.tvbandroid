...
def clear_archive():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        ...
                        yeslabel='[B][COLOR springgreen]Clear[/COLOR][/B]'):
...
def toggle_cache(state):
    ...
    titlelist = ['Include Video Add-ons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include NuMb3r5', 'Include Scrubs v2', 'Include Seren', 'Include THE CREW', 'Include Venom']
        ...
        if state not in ['includevideo', 'includeall'] and CONFIG.get_setting('includeall') == 'true':
            try:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                              "[COLOR {0}]You will need to turn off [COLOR {1}]Include All Add-ons[/COLOR] to disable[/COLOR] [COLOR {2}]{3}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1, item))
            except:
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] add-on data stored in your userdata folder?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] add-on data stored in your userdata folder for uninstalled add-ons?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[B][COLOR springgreen]Remove Data[/COLOR][/B]',
                            nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] empty add-on data folders in your userdata folder?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[B][COLOR springgreen]Remove Data[/COLOR][/B]',
                            nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
...
def remove_addon_menu():
    ...
    if len(addonnames) == 0:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]No add-ons to remove![/COLOR]".format(CONFIG.COLOR2))
        return
    selected = dialog.multiselect("{0}[COLOR red]:[/COLOR] [COLOR gold]Select the add-ons you want to remove![/COLOR]".format(CONFIG.ADDONTITLE), addonnames)
    if not selected:
        return
    if len(selected) > 0:
        ...
        dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]To save your changes, you now need to Force Close Kodi.\n\nClick OK to continue![/COLOR]".format(CONFIG.COLOR2))
