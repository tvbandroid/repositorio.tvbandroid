        ...
        if TEMPAPKFILE:
            ...
            if len(match) > 0:
                ...
                for aname, section, url, icon, fanart, adult, description in match:
                    ...
                    if x == 0:
                        directory.add_file("No add-ons added to this menu yet!", themeit=CONFIG.THEME2)
            else:
            ...
            if len(match) > 0:
                for name, section, url, icon, fanart, description in match:
                    if section.lower() == "yes":
                        ...
                        directory.add_file(name, {'name': name, 'mode': 'viewVideo', 'url': url, 'icon': icon, 'description': description}, description=description, icon=icon, fanart=fanart, themeit=CONFIG.THEME2)
            else:
...
def system_info():
    ...
    directory.add_file('[COLOR {0}]Video Add-ons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(video))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Program Add-ons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(programs))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Music Add-ons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(music))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Picture Add-ons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(picture))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
...
def save_menu():
    ...
    directory.add_file('[COLOR gold]- Click to toggle settings -[/COLOR]', themeit=CONFIG.THEME3)
    ...
    directory.add_file('Keep \'guisettings.xml\': {0}'.format(guisettings.replace('true', on).replace('false', off)), {'mode': 'togglesetting', 'name': 'keepguisettings'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
...
def login_menu():
    ...
    directory.add_file('[I]Several of these add-ons are PAID services.[/I]', icon=CONFIG.ICONLOGIN, themeit=CONFIG.THEME3)
...
def enable_addons(all=False):
    ...
    if not all:
        if len(addonids) == 0:
            directory.add_file("No Add-ons Found to Enable or Disable.", icon=CONFIG.ICONMAINT)
        else:
            directory.add_file("[I][B][COLOR red]!!Notice: Disabling Some Add-ons Can Cause Issues!![/COLOR][/B][/I]", icon=CONFIG.ICONMAINT)
            directory.add_dir('[COLOR gold]Enable All Add-ons[/COLOR]', {'mode': 'enableall'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
...
def remove_addon_data_menu():
    if os.path.exists(CONFIG.ADDON_DATA):
        directory.add_file('[COLOR red][B][REMOVE][/B][/COLOR] All Add-on_Data', {'mode': 'removedata', 'name': 'all'}, themeit=CONFIG.THEME2)
        directory.add_file('[COLOR red][B][REMOVE][/B][/COLOR] All Add-on_Data for Uninstalled Add-ons', {'mode': 'removedata', 'name': 'uninstalled'}, themeit=CONFIG.THEME2)
