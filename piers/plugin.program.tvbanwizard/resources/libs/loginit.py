...
LOGINID = {
    ...
    'fanart-tmdbhelper': {
        ...
        'activate' : ''},
    'login-youtube': {
        'name'     : 'Personal API - [COLOR white]You[COLOR red]Tube',
        'saved'    : 'login-youtube',
    ...
    if who == 'all':
        for log in ORDER:
            if os.path.exists(LOGINID[log]['path']):
                ...
                logging.log('[Información de Inicio de Sesión] {0}({1}) no está instalado'.format(LOGINID[log]['name'], LOGINID[log]['plugin']), level=xbmc.LOGERROR)
        ...
        if LOGINID[who]:
            ...
            logging.log('[Información de Inicio de Sesión] Entrada Inválida: {0}'.format(who), level=xbmc.LOGERROR)
    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, LOGINID[who]['name']),
                               '[COLOR {0}]Información de Inicio de Sesión: Eliminado![/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               LOGINID[who]['icon'])
    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Datos de Inicio de Sesión guardados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Datos de Inicio de Sesión] No se Puede Actualizar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Datos de Inicio de Sesión No Registrados para {0}'.format(name))
    elif do == 'restore':
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Datos de Inicio de Sesión Restaurados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Información de Inicio de Sesión] No se Puede Restaurar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Datos de Inicio de Sesión No Encontrados para {0}'.format(name))
    elif do == 'clearaddon':
            ...
            try:
                ...
                for setting in root.findall('setting'):
                    if setting.attrib['id'] in data:
                        logging.log('Quitar Configuración: {0}'.format(setting.attrib))
                        root.remove(setting)
                            
                tree.write(settings)
                
                logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                                   '[COLOR {0}]Datos del Addon: Borrados![/COLOR]'.format(CONFIG.COLOR2),
                                   2000,
                                   icon)
            except Exception as e:
                logging.log("[Datos de Trakt] No se Puede Borrar el Addon {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
    xbmc.executebuiltin('Container.Refresh()')
        ...
        if os.path.exists(LOGINID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Desea guardar la [COLOR {0}]Información de Inicio de Sesión[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"[COLOR khaki]Addon: [COLOR springgreen][B]{0}[/B][/COLOR]".format(u)
                                    +'\n'+"[COLOR yellogreen]Guardado:[/COLOR] [COLOR red][B]{0}[/B][/COLOR]".format(su) if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Ninguno[/B][/COLOR]',
                                    yeslabel="[B][COLOR cyan]Guardar Datos[/COLOR][/B]",
...
def activate_login(who):
    if LOGINID[who]:
        if os.path.exists(LOGINID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} no está instalado actualmente.'.format(LOGINID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
