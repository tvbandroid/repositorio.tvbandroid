...
def binaries():
    ...
    if os.path.exists(binarytxt):
        ...
        logging.log("[Binary Detection] Reinstalling Eligible Binary Add-ons")
        dialog.ok(CONFIG.ADDONTITLE,
                  '[COLOR {0}]The restored build contains platform-specific add-ons, which will now be '
        ...
        logging.log("[Binary Detection] No Eligible Binary Add-ons to Reinstall")
        return True
    ...
    if len(binaryids) == 0:
        logging.log('No add-ons selected for installation.')
        return
    ...
    if not fail:
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR gold]The selected add-ons were all installed successfully![/COLOR]')
        os.remove(binarytxt)
        return True
    else:
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR gold]The following add-ons failed to install:[/COLOR]\n{0}'.format(', '.join(fail)))
        return False
...
class Restore:
    ...
    def _from_file(self, file, loc):
        ...
        self.progress_dialog.update(0, 'Installing External Back-up' + '\n' + 'Please Wait')
    ...
    def choose(self, location):
        ...
        file = self.dialog.browseSingle(1, '[COLOR {0}]Select the back-up file you want to restore[/COLOR]'.format(
        ...
        self.progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]Installing {1} Back-up'.format(CONFIG.COLOR2, external) + '\n' + 'Please Wait[/COLOR]')
