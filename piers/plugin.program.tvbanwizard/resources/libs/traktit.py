    ...
    if who == 'all':
        for log in ORDER:
            if os.path.exists(TRAKTID[log]['path']):
                ...
                logging.log('[Datos de Trakt] {0}({1}) no está instalado'.format(TRAKTID[log]['name'], TRAKTID[log]['plugin']), level=xbmc.LOGERROR)
        ...
        if TRAKTID[who]:
            ...
            logging.log('[Datos de Trakt] Entrada Inválida: {0}'.format(who), level=xbmc.LOGERROR)
    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, TRAKTID[who]['name']),
                               '[COLOR {0}]Datos de Trakt: Eliminado![/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               TRAKTID[who]['icon'])
    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Datos de Trakt Guardados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Datos de Trakt] No se Puede Actualizar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Datos de Trakt No Registrados para {0}'.format(name))
    elif do == 'restore':
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Datos de Trakt Restaurados para {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[Datos de Trakt] No se Puede Restaurar {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('Datos de Trakt No Encontrados para {0}'.format(name))
    elif do == 'clearaddon':
        ...
        if os.path.exists(TRAKTID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Desea guardar los [COLOR {0}]Datos de Trakt[/COLOR] para [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(TRAKTID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]Datos de Trakt: Importado![/COLOR]'.format(CONFIG.COLOR2))
...
def activate_trakt(who):
    if TRAKTID[who]:
        if os.path.exists(TRAKTID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} no está instalado actualmente.'.format(TRAKTID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
