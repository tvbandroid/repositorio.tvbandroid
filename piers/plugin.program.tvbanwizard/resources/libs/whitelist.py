...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        while choice not in [-1, 0]:
            choice = dialog.select("{0}[COLOR red]:[/COLOR] [COLOR gold]Select the add-ons to add to your Whitelist![/COLOR]".format(CONFIG.ADDONTITLE), tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
                break
        ...
        if len(list) > 0:
            msg = "[COLOR gold]Here is a list of your Whitelist plugins. These items (along with dependencies) will not be removed when preforming a fresh start, or when the userdata is overwritten in a build install![/COLOR][CR][CR]"
            for item in list:
                try:
                    name, id, fold = item
                except Exception as e:
        ...
        source = dialog.browse(3,
                                   '[COLOR {0}]Select where to export the Whitelist file![/COLOR]'.format(CONFIG.COLOR2),
                                   'files', '.txt', False, False, CONFIG.HOME)
        logging.log(str(source))
        try:
        ...
        if not dialog.yesno(CONFIG.ADDONTITLE,
                                ...
                                +'\n\n'+"This process cannot be undone![/COLOR]",
                                yeslabel="[B][COLOR springgreen]Remove[/COLOR][/B]",
                                nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
            logging.log_notify(CONFIG.ADDONTITLE,
