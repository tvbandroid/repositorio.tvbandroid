...
class Wizard:
    ...
    def gui(self, name, over=False):
        ...
        if yes_pressed:
            ...
            if not response:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]GuiFix: [/COLOR] [COLOR gold]Url Zip Inválido![/COLOR]'.format(CONFIG.COLOR2))
                return

            self.dialogProgress.create(CONFIG.ADDONTITLE, '[COLOR {0}][B]Descargando GuiFix:[/B][/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name) + '' + 'Espere por Favor')
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               '[COLOR {0}]GuiFix: [COLOR gold]Cancelado![/COLOR][/COLOR]'.format(CONFIG.COLOR2))
    ...
    def theme(self, name, theme='', over=False):
        ...
        if installtheme:
            ...
            if test1:
                skin.look_and_feel_data('save')
				xbmc.log('test1= ' + str(test2), xbmc.LOGINFO)
