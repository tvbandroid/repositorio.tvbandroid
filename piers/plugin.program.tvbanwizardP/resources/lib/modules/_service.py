...
from .addonvar import setting, setting_set, addon_name, addon_icon, addon_id, addon_profile, isBase64, headers, dialog, local_string, gui_save_default, kodi_ver_, get_page, UPDATE_VERSION, CURRENT_BUILD, CURRENT_VERSION, BUILD_URL, BUILD_NAME, OLD_BUILD
           ...
           if UPDATE_VERSION is None:
               ...
               if UPDATE_VERSION > CURRENT_VERSION and setting('update_passed') != 'true' and CURRENT_BUILD == OLD_BUILD:
                   update_available = xbmcgui.Dialog().yesnocustom(
                       ...
                       yeslabel='Update Now', nolabel='Not Now', customlabel='Changelog', defaultbutton=xbmcgui.DLG_YESNO_YES_BTN
                   )
    ...
    def new_builds_notify(self):
        ...
        for build in builds:
            if not build.get('version'):
                pass
            elif kodi_ver_ in build.get('kodi'):
                    current_list.append(build.get('name'))
        if not xbmcvfs.exists(file_path) and current_list:
            ...
            with open(file_path, 'r') as archivedlist:
                archived_list = json.load(archivedlist)['builds']
                if current_list == archived_list:
                    return
                elif current_list and len(archived_list) > len(current_list):
                    with open(file_path, 'w') as buildlist:
                        json.dump({'builds': current_list}, buildlist, indent = 4)
                else:
                    a = set(current_list)
                    ...
                    if notify == 1:
                        with open(file_path, 'w') as buildlist:
                            json.dump({'builds': current_list}, buildlist, indent = 4)
                        xbmc.executebuiltin("ActivateWindow(10001,plugin://%s/?mode=1,return)" % addon_id)
                    elif notify == 0:
                        with open(file_path, 'w') as buildlist:
                            json.dump({'builds': current_list}, buildlist, indent = 4)
                    elif notify == 2:
