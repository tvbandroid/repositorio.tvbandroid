...
from .addonvar import addon_name, setting, addon_icon, addon_fanart, local_string, authorize, kodi_ver, kodi_versions, UPDATE_VERSION, CURRENT_BUILD, BUILD_VERSION, BUILD_NAME, NUM_BUILDS
...
def main_menu():
    ...
    add_dir(COLOR4(f'Kodi Version:  v{kodi_ver}'), '', '', addon_icon, addon_fanart, COLOR2('Installed Version of Kodi'), isFolder=False)  # Kodi Version
    if UPDATE_VERSION > BUILD_VERSION:
        add_dir(COLOR3(f'[B]Build Update Available!!![/B]   [{BUILD_NAME} v{UPDATE_VERSION}]'), '', 32, addon_icon, addon_fanart, COLOR2(local_string(30110)), isFolder=False)  # Build Update Available
    ...
    add_dir(COLOR2(local_string(30026)),'',10,addon_icon,addon_fanart,COLOR2('Integrates with Account Manager from the 7o9 Repo'), isFolder=False)  # Authorize Debrid Services
    ...
    if videos_url not in ('', 'http://', 'http://CHANGEME'):
        add_dir(COLOR2('Videos'), videos_url, 30, addon_icon, addon_fanart, COLOR2('TVban Wiazrd Wizard TV'), isFolder=True) # Videos
...
def build_menu():
    ...
    try:
       ...
       xbmcgui.Dialog().notification(addon_name, '[COLOR gold]No Build File Present. Please contact[/COLOR] [COLOR red]TVban Wizard[/COLOR][COLOR gold]![/COLOR]', addon_icon, 3000)
       quit()
...
def submenu_maintenance():
    ...
    add_dir(COLOR1('<><> [B]Clean Kodi[/B] <><>'),'','',addon_icon,addon_fanart, COLOR1('*** Kodi Cleaning Tools ***'),isFolder=False)
    ...
    add_dir(COLOR1('<><> [B]Wizard Tools[/B] <><>'),'','',addon_icon,addon_fanart, COLOR1('*** TVban Wiazrd Tools ***'),isFolder=False)
    ...
    add_dir(COLOR2('View Logs'),'', 26, addon_icon,addon_fanart,COLOR2('View Kodi Logs'), isFolder=False)
