...
class API:
    ...
    # ======================
    # FILMES (VOD) – JSON (Player API)
    # ======================
    def vod_cat(self):
        itens = []
        url_vod = '{0}&action=get_vod_categories'.format(self.player_api)
        vod_cat = self.http(url_vod,'json_url')
        if vod_cat:
            for cat in vod_cat:
                name = cat['category_name']
                url = self.player_api + '&action=get_vod_streams&category_id='+cat['category_id']
                if self.hide_adult == 'false':
                    itens.append((name,url))
                else:
                    if not any(s in name for s in self.adult_tags):
    ...
    def vod_list(self, url):
        itens = []
        vod_list = self.http(url,'json_url')
        if vod_list:
            for vod in vod_list:
                try:
                    name = vod['name']
                    stream_id = vod['stream_id']
                    url_ = self.play_movies + str(stream_id) + '.' + vod['container_extension']
                    thumb = vod.get('stream_icon','')
                    plot = vod.get('plot','')
                    releasedate = vod.get('releasedate','')
                    cast = str(vod.get('cast','')).split()
                    rating = vod.get('rating','')
                    duration = str(vod.get('duration',''))
                    genre = vod.get('genre','')
                    background = vod.get('backdrop_path',[ ''])[0] if vod.get('backdrop_path') else ''
                    itens.append((name,url_,thumb,background,plot,releasedate,cast,rating,duration,genre))
                except:
                    pass
        return itens

    # ======================
    # SÉRIES – JSON (Player API)
    # ======================
    def series_cat(self):
        itens = []
    ...
    def season_list(self,url):
        ...
        ser_cat = self.http(url,'json_url')
        info = ser_cat['info']
        ser_cat_eps = ser_cat['episodes']
        parsed_url = urlparse(url)
        season_number = str(parse_qs(parsed_url.query)['season_number'][0])
        for ser in ser_cat_eps[season_number]:
            url_play = self.play_series+str(ser['id'])+'.'+ser['container_extension']
            name = ser['title']
            try:
                thumb = ser['info'].get('movie_image','')
            except:
                thumb = ''
            try:
                background = ser['info'].get('movie_image','')
            except:
                background = ''
            try:
                plot = ser['info'].get('plot','')
            except:
                plot = ''
            try:
                releasedate = ser['info'].get('releasedate','')
            except:
                releasedate = ''
            try:
            ...
            try:
                ...
                rating_5based = ''
            try:
                duration = str(ser['info'].get('duration',''))
            except:
                duration = ''
            try:
                ...
                genre = ''
            itens.append((name,url_play,thumb,background,plot,releasedate,cast,rating_5based,duration,genre))
        return itens 

    # ======================
    # FILMES (VOD) – XML (modo legado)
    # ======================
    def vod(self,url=''):
        # exemplo no addon
        # tipo = t[0]
        ...
        itens = []
        if not url:
            open_ = self.http(self.vod_url, mode='vod')
        else:
            open_ = self.http(url, 'vod')
        if open_:
            all_cats = self.regex_get_all(open_,'<channel>','</channel>')
            if all_cats:
                for a in all_cats:
                    # correção: checar dentro do bloco 'a', não no documento inteiro
                    if '<playlist_url>' in a:
                        name = str(self.b64(self.regex_from_to(a,'<title>','</title>'))).replace('?','')
                        urlp = self.check_protocol(self.regex_from_to(a,'<playlist_url>','</playlist_url>').replace('<![CDATA[','').replace(']]>',''))
                        if not 'All' in name: 
                            if self.hide_adult == 'false':
                                itens.append(('dir', name,urlp))
                            else:
                                if not any(s in name for s in self.adult_tags):
                                    itens.append(('dir', name,urlp))
                    else:
                        try:
                            ...
                            urlp = self.check_protocol(self.regex_from_to(a,'<stream_url>','</stream_url>').replace('<![CDATA[','').replace(']]>',''))
                            ...
                            background = ''
                            itens.append(('play', name, urlp, thumb, background, plot, str(year).replace("['", "").replace("']", ""), str(cast).split(), ratin, genre))
                        except:
                            pass
        return itens