        ...
        for command in split_commands:
            ...
            if len(command) > 0:
                ...
                if "=" in command:
                    ...
                    value = parse.unquote_plus(split_command[1])
    ...
    # one of the two cookie handling modules
        ...
        if cookielib is not None:
            ...
            if not follow_redirects:
                ...
                opener = request.build_opener(request.HTTPHandler(debuglevel=http_debug_log_enabled),request.HTTPCookieProcessor(cj),NoRedirectHandler())

            else:

                opener = request.build_opener(request.HTTPHandler(debuglevel=http_debug_log_enabled),request.HTTPCookieProcessor(cj))
...
def add_item( action="" , title="" , plot="" , url="" , thumbnail="" , fanart="" , show="" , episode="" , extra="", page="", info_labels = None, isPlayable = False , folder=True ):
    ...
    if url.startswith("plugin://"):
        ...
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % ( sys.argv[ 0 ] , action , parse.quote_plus( title ) , parse.quote_plus(url) , parse.quote_plus( thumbnail ) , parse.quote_plus( plot ) , parse.quote_plus( extra ) , parse.quote_plus( page ))
