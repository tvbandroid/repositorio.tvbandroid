...
def main_menu():
    ...
    add_dir(COLOR2(local_string(30026)),'',10,addon_icon,addon_fanart,COLOR2(local_string(30026)), isFolder=False)  # Authorize Debrid Services
    ...
    if videos_url not in ('', 'http://', 'http://CHANGEME'):
        add_dir(COLOR2('Videos'), videos_url, 30, addon_icon, addon_fanart, COLOR2('The Red Wizard TV'), isFolder=True) # Videos
...
def submenu_maintenance():
    ...
    add_dir(COLOR1('<><> [B]Wizard Tools[/B] <><>'),'','',addon_icon,addon_fanart, COLOR1('*** The Red Wizard Tools ***'),isFolder=False)
