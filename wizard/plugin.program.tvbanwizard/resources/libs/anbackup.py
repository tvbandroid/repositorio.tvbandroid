...
def cleanup_backup():
    ...
    for item in sorted(folder, key=os.path.getmtime):
        ...
        if os.path.isdir(item):
            ...
            list.append(base)
    list = ['--- [COLOR yellowgreen] QUITAR TODOS LOS ELEMENTOS[/COLOR] ---'] + list
    selected = dialog.select("{0}: Seleccione los que desee eliminar de la carpeta 'Mis_Builds'.".format(CONFIG.ADDONTITLE),
                             list)
