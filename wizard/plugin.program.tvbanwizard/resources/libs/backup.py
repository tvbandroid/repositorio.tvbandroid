...
def cleanup_backup():
    ...
    for item in sorted(folder, key=os.path.getmtime):
        ...
        if os.path.isdir(item):
            ...
            list.append(base)
    list = ['[B] --- [COLOR tomato] QUITAR TODOS LOS ELEMENTOS[/COLOR] ---[/B]'] + list
    selected = dialog.select("{0}: [COLOR yellowgreen]Seleccione archivos de la carpeta 'Mis_Builds'[/COLOR] ".format(CONFIG.ADDONTITLE),
                             list)
